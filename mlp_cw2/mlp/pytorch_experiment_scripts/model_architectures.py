...
class ConvolutionalProcessingBlock_bn_rc(nn.Module):
    def __init__(self, input_shape, num_filters, kernel_size, padding, bias, dilation, stride=1, identity_downsample=None):
...
class ConvolutionalDimensionalityReductionBlock_bn_rc(nn.Module):
    def __init__(self, input_shape, num_filters, kernel_size, padding, bias, dilation, reduction_factor,stride=1, identity_downsample=None):
    ...
    def forward(self, x):
        ...
        out = F.avg_pool2d(out, self.reduction_factor)
        identity = F.avg_pool2d(identity, self.reduction_factor)
        #needs to be shrinked to match size of out so pooling is used on residual
        out = self.layer_dict['conv_1'].forward(out)
        out = self.layer_dict['bn_1'](out)
        if self.identity_downsample is not None:
            identity = self.identity_downsample(identity)
