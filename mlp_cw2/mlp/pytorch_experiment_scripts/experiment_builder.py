...
class ExperimentBuilder(nn.Module):
    ...
    def plot_grad_flow(self, named_parameters):
        ...
        Recei model being trained. Returns plot of gradient flow for the given modmeters of the
       el parameters.ves the para
        """
        all_grads = []
        layers = []
        ...
        for name, value in named_parameters:
            if "bias" not in name and value.requires_grad:
                layers.append(name[11:-7].replace('.layer_dict.', '_'))
                all_grads.append(value.grad.abs().mean())
        layers[-1] = 'weight_logit_linear_layer'
        ########################################
